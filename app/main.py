...
def add_predictions(input_data):
    ...
    st.write("Probability of being Balign: ", model.predict_proba(input_array_scaled)[0][0])
    st.write("Probability of being Malignant: ", model.predict_proba(input_array_scaled)[0][1])
    st.write("This app can help professionals in making a diagnosis, but should not be used as a subtitute for a professional diagnosis")
...
def main():
    ...
    with st.container():
        st.title("Breast Cancer Predator")
        st.markdown("<style>div.block-container{padding-top:2rem;}</style>", unsafe_allow_html=True)
        st.write("Please connect this app to your cythology lab to help diagnose breast cancer from your tissue sample. This app predicts, using a machine learning model, whether a breast mass is benign or malignant based on the measurements it receives from the cythosis Laboratory. You can also update the measurements by hand using the sliders in the sidebar. ")
